...
def _parse_route(r: dict) -> OSRMRoute:
    route = OSRMRoute(
        ...
        weight_name=_parse_enum(OSRMWeightName, r.get("weight_name")),
        legs=[_parse_leg(l) for l in r.get("legs", [])]
    )
...
def _parse_leg(l: dict) -> OSRMLeg:
    return OSRMLeg(
        ...
        steps=[_parse_step(s) for s in l.get("steps", [])]
    )
...
def _parse_step(s: dict) -> OSRMStep:
    ...
    return OSRMStep(
        ...
        mode=_parse_enum(OSRMMode, s.get("mode")),
        maneuver=_parse_maneuver(s.get("maneuver")),
        intersections=[_parse_intersection(i) for i in s.get("intersections", [])],
        rotary_name=s.get("rotary_name", ""),
        rotary_pronunciation=s.get("rotary_pronunciation", ""),
        driving_side=_parse_enum(OSRMDrivingSide, s.get("driving_side"))
    )
...
def _parse_maneuver(m: dict) -> OSRMManeuver:
    ...
    return OSRMManeuver(
        ...
        type=_parse_enum(OSRMManeuverType, m.get("type")),
        modifier=_parse_enum(OSRMModifier, m.get("modifier")),
    ...
    if "lanes" in i and isinstance(i["lanes"], list):
        for l in i["lanes"]:
            lanes.append(OSRMLane(
                ...
                indications=[_parse_enum(OSRMIndication, ind) for ind in l.get("indications", [])]
            ))
